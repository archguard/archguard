...
class MetricsServiceImplTest {
    ...
    fun `should get module coupling`() {
        ...
        every { logicModuleRepository.getAllBysystemId(systemId) } returns listOf(element, element2, element3)
        every { dependencyService.getAllClassDependencies(systemId) } returns listOf(dependency1, dependency2)
        val slot = slot<List<ModuleMetricsLegacy>>()
        every { metricsRepository.insert(capture(slot)) } just Runs

        service.calculateCouplingLegacy(systemId)
    ...
    fun `should be zero when no dependence`() {
        ...
        every { logicModuleRepository.getAllBysystemId(systemId) } returns listOf(element, element2)
        every { dependencyService.getAllClassDependencies(systemId) } returns listOf()
        val slot = slot<List<ModuleMetricsLegacy>>()
        every { metricsRepository.insert(capture(slot)) } just Runs
    ...
    fun `should get all metrics by module name`() {
        ...
        every { logicModuleRepository.getAllByShowStatus(systemId, true) } returns listOf(element)
        val slot = slot<List<String>>()
        every { metricsRepository.findAllMetrics(capture(slot)) } answers { listOf(allMetrics) }

        val result = service.getAllMetricsLegacy(systemId)
