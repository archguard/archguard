...
interface ClassCouplingReadDao {
    @SqlQuery(
        "select c.class_id, c.system_id, j.name, j.module, " +
            "c.inner_fan_in, c.inner_fan_out, c.outer_fan_in, c.outer_fan_out " +
            "from metric_class_coupling c join code_class j where c.class_id = j.id and j.id = :classId"
    )
    ...
    @SqlQuery(
        "select c.class_id, c.system_id, j.name, j.module, " +
            "c.inner_fan_in, c.inner_fan_out, c.outer_fan_in, c.outer_fan_out " +
            "from metric_class_coupling c join code_class j where c.class_id = j.id and j.id in (<classIds>)"
    )
