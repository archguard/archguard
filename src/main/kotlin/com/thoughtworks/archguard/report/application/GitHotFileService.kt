...
@Service
class GitHotFileService(val gitHotFileRepo: GitHotFileRepo, @Value("\${git_hot_file.modified_count_baseline}") val modifiedCountBaseline: Int) {
    fun getGitHotFilesBySystemId(systemId: Long) : List<GitHotFile> {
        return gitHotFileRepo.findBySystemId(systemId).filter { (it.jclassId != null) && (it.modifiedCount >= modifiedCountBaseline) }
    }
}
