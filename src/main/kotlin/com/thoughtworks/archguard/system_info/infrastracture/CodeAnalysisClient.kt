...
@Component
class CodeAnalysisClient(@Value("\${module.client.host}") val url: String) : AnalysisClientProxy {

    private val log = LoggerFactory.getLogger(CodeAnalysisClient::class.java)

    override fun refreshThresholdCache() {
        try {
            RestTemplate().postForLocation("$url/bad-smell-threshold/reload", null, null)
