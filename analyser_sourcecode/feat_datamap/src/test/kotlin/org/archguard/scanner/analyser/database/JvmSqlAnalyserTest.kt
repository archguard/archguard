...
internal class JvmSqlAnalyserTest {
    @Test
    fun should_wrapper_in_list_in_values() {
        val sqlify =
            JvmSqlAnalyser().sqlify("select id, system_name as systemName, language from system_info where id in (<ids>)")
    ...
    fun should_handle_variable_in_sql() {
        val sqlify = JvmSqlAnalyser().sqlify("select id, module_name from \"\\\"+orderSqlPiece+\"\\\"")
        assertEquals("select id, module_name from *", sqlify)
    }

    @Test
    fun should_handle_plus_without_double_quote() {
        val sqlify = JvmSqlAnalyser().sqlify("select id, system_name as systemName, language from system_info +\"")
        assertEquals("select id, system_name as systemName, language from system_info ", sqlify)
    }

    @Test
    fun should_kotlin_variable_in_sql() {
        val sqlify = JvmSqlAnalyser().sqlify("select id, module_name from system and c.name = '${'$'}name'")
    ...
    fun should_handle_end_with_string_append() {
        val sqlify = JvmSqlAnalyser().sqlify("select id, module_name from system and c.name = ${'$'}name\"+moduleFilter")
        assertEquals("select id, module_name from system and c.name = ''", sqlify)
    }
    ...
    fun should_handle_kotlin_string_in_sql() {
        val sqlify =
            JvmSqlAnalyser().sqlify("SELECT id, name, module, loc, access FROM code_class where system_id = '' and name = '' '' limit ''")
        assertEquals(
