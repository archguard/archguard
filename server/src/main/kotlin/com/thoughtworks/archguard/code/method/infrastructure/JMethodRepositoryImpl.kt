...
class JMethodRepositoryImpl(val jdbi: Jdbi) : JMethodRepository {
    ...
    override fun findMethodFields(id: String): List<JField> {
        val sql =
            "SELECT id, name, type FROM code_field WHERE id in (select b from code_ref_method_fields where a='$id')"
    ...
    override fun findMethodsByModuleAndClass(systemId: Long, module: String?, name: String): List<JMethod> {
        val sql =
            "SELECT id, name, clzname as clazz, module, returntype, argumenttypes, access FROM code_method WHERE clzname='$name' AND system_id='$systemId' AND module <=>'$module'"
    ...
    override fun findMethodCallers(id: String): List<JMethod> {
        val sql =
            "SELECT id, name, clzname as clazz, module, returntype, argumenttypes, access  FROM code_method WHERE id IN (SELECT a FROM code_ref_method_callees WHERE b='$id') "
        return jdbi.withHandle<List<JMethod>, Nothing> {
            it.registerRowMapper(ConstructorMapper.factory(JMethod::class.java))
            it.createQuery(sql)
                .mapTo(JMethodDto::class.java)
    ...
    override fun findMethodCallees(id: String): List<JMethod> {
        val sql =
            "SELECT id, name, clzname as clazz, module, returntype, argumenttypes, access  FROM code_method WHERE id IN (SELECT b FROM code_ref_method_callees WHERE a='$id') "
        return jdbi.withHandle<List<JMethod>, Nothing> {
            it.registerRowMapper(ConstructorMapper.factory(JMethod::class.java))
            it.createQuery(sql)
                .mapTo(JMethodDto::class.java)
    ...
    override fun findMethodByModuleAndClazzAndName(
        systemId: Long,
        moduleName: String?,
        clazzName: String,
        methodName: String
        ...
        val sql =
            "SELECT id, name, clzname as clazz, module, returntype, argumenttypes, access FROM code_method WHERE " +
