...
class AccessRepositoryImpl(val jdbi: Jdbi) : AccessRepository {
    ...
    private fun save(clazz: ClassAccess, systemId: Long) {
        jdbi.useHandle<Exception> {
            it.createUpdate(
                "insert into code_class_access (id, class_id, is_abstract, is_interface, system_id) " +
    ...
    private fun save(method: MethodAccess, systemId: Long) {
        jdbi.useHandle<Exception> {
            it.createUpdate(
                "insert into method_access (id, method_id, is_abstract, is_synthetic, is_static, is_private, system_id) " +
