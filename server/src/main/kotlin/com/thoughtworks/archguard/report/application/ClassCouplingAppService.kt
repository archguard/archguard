package com.thoughtworks.archguard.report.application

import com.thoughtworks.archguard.report.domain.coupling.hub.ClassCoupling
import com.thoughtworks.archguard.report.domain.coupling.hub.ClassCouplingRepository
import org.archguard.gate.ComparatorOperator
import org.archguard.gate.LayerType
import com.thoughtworks.archguard.report.domain.qualitygate.QualityGateClient
import org.archguard.gate.QualityGateConfig
import org.springframework.stereotype.Service

@Service
class ClassCouplingAppService(val classCouplingRepository: ClassCouplingRepository, val qualityGateClient: QualityGateClient) {
    fun getCouplingFilterByQualityGate(systemId: Long, qualityGateName: String): List<ClassCoupling> {
        val allCoupling = classCouplingRepository.getAllCoupling(systemId)
        val couplingQualityGate = qualityGateClient.getQualityGate(qualityGateName)
        val config = couplingQualityGate.config ?: return allCoupling

        return filterByQualityGateConfig(config, allCoupling)
    }

    private fun filterByQualityGateConfig(qualityGateConfigs: List<QualityGateConfig>, couplings: List<ClassCoupling>): List<ClassCoupling> {
        var duplicatedCouplings = couplings
        val fanIn = qualityGateConfigs.firstOrNull { it.layer == LayerType.COUPLINGS && it.quota == "fanIn" }
        if (fanIn != null) {
            duplicatedCouplings = when (fanIn.operator) {
                ComparatorOperator.BIGGER -> duplicatedCouplings.filter { it.fanIn < fanIn.value.toInt() }
                ComparatorOperator.EQUAL -> duplicatedCouplings.filter { it.fanIn != fanIn.value.toInt() }
                ComparatorOperator.LESS -> duplicatedCouplings.filter { it.fanIn > fanIn.value.toInt() }
            }
        }
        val fanOut = qualityGateConfigs.firstOrNull { it.layer == LayerType.COUPLINGS && it.quota == "fanOut" }
        if (fanOut != null) {
            duplicatedCouplings = when (fanOut.operator) {
                ComparatorOperator.BIGGER -> duplicatedCouplings.filter { it.fanOut < fanOut.value.toInt() }
                ComparatorOperator.EQUAL -> duplicatedCouplings.filter { it.fanOut != fanOut.value.toInt() }
                ComparatorOperator.LESS -> duplicatedCouplings.filter { it.fanOut > fanOut.value.toInt() }
            }
        }
        val coupling = qualityGateConfigs.firstOrNull { it.layer == LayerType.COUPLINGS && it.quota == "coupling" }
        if (coupling != null) {
            duplicatedCouplings = when (coupling.operator) {
                ComparatorOperator.BIGGER -> duplicatedCouplings.filter { it.coupling < coupling.value.toDouble() }
                ComparatorOperator.EQUAL -> duplicatedCouplings.filter { it.coupling != coupling.value.toDouble() }
                ComparatorOperator.LESS -> duplicatedCouplings.filter { it.coupling > coupling.value.toDouble() }
            }
        }
        val instability = qualityGateConfigs.firstOrNull { it.layer == LayerType.COUPLINGS && it.quota == "instability" }
        if (instability != null) {
            duplicatedCouplings = when (instability.operator) {
                ComparatorOperator.BIGGER -> duplicatedCouplings.filter { it.instability < instability.value.toDouble() }
                ComparatorOperator.EQUAL -> duplicatedCouplings.filter { it.instability != instability.value.toDouble() }
                ComparatorOperator.LESS -> duplicatedCouplings.filter { it.instability > instability.value.toDouble() }
            }
        }
        return duplicatedCouplings
    }
}
