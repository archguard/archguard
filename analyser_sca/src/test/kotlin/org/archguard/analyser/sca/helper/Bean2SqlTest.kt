...
internal class Bean2SqlTest {
    ...
    fun normal_bean() {
        ...
        assertEquals("insert into project_composition_dependencies(dep_artifact, dep_group, dep_metadata, dep_name, dep_scope, dep_source, id, name, package_manager, parent_id, system_id, version) values ('', '', '', '', '', '', '12', '', '', '', '', '');",
            sqls)
    }
    ...
    fun two_beans() {
        ...
        val splits = sqls.split("\n")
        assertEquals(1, splits.size)
        assertEquals("insert into project_composition_dependencies(dep_artifact, dep_group, dep_metadata, dep_name, dep_scope, dep_source, id, name, package_manager, parent_id, system_id, version) values ('', '', '', '', '', '', '1', '', '', '', '', ''),('', '', '', '', '', '', '2', '', '', '', '', '');",
            sqls)
    }
